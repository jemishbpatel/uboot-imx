...
#ifdef CONFIG_MV88E6176_CMD
int do_mvsw_reg_dumpall(const char *name, int argc, char * const argv[])
{
	...
	if (ret) {
        ...
        for(port=0; port<5; port++) {
                for(reg=0; reg<32; reg++) {
                        ...
                        if (ret) {
                                printf("%s: Error configuring switch\n", __func__);
                                ppu_enable(name, phyaddr);
                                return ret;
                        }
                        printf("port=0x%x reg=%d value=0x%x\n", port, reg, value);
                }
        }
	...
	printf("Global-1 Registers:\n");
	for(reg=0; reg<32; reg++) {
		ret = sw_reg_read(name, phyaddr, 0x1b, reg, &value);
		if (ret) {
			printf("%s: Error configuring switch\n", __func__);
			ppu_enable(name, phyaddr);
			return ret;
		}
		printf("Global-1 reg=%d value=0x%x\n", reg, value);
	}
        printf("Global-2 Registers:\n");
        for(reg=0; reg<32; reg++) {
                ret = sw_reg_read(name, phyaddr, 0x1c, reg, &value);
                if (ret) {
                        printf("%s: Error configuring switch\n", __func__);
                        ppu_enable(name, phyaddr);
                        return ret;
                }
                printf("Global-2 reg=%d value=0x%x\n", reg, value);
        }

	/* re-enable the PPU */
	ret = ppu_enable(name, phyaddr);
	if (ret) {
		printf("%s: Error enabling PPU\n", __func__);
		return ret;
	}
...
int do_mvsw_reg_dump(const char *name, int argc, char * const argv[])
{
	...
	if(count > 32 ) {
		printf("count exceeds no of register's limit\n");
		return -1;
	}
...
int do_mvsw_reg(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(cmd, "read") == 0) {
		...
		printf("argc = %d\n", argc);
		if(argc == 2) {
			ret = do_mvsw_reg_dumpall(ethname, argc, argv);
		}
		else if(argc == 4) {
			ret = do_mvsw_reg_dump(ethname, argc, argv);
		}
	} else
		return cmd_usage(cmdtp);
...
U_BOOT_CMD(
	...
	"marvell 88e6176 switch register access",
	"mvsw_reg write ethname phyaddr port reg value\n"
	"mvsw_reg read ethname phyaddr port reg\n"
	"mvsw_reg dump ethname phyaddr port count\n"
	"mvsw_reg dump ethname phyaddr\n"
	);
#endif
